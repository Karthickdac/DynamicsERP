...
- **Admin** — Company Settings (single-row, used in PDFs/emails), Users (full CRUD + role/active filters + password reset), Staff (HR directory, manual + MysticsHR live sync via `GET /api/v1/employees`, paginated, upserts on `(source='mysticshr', externalId)`), Email Templates (system + custom, double-curly placeholders, per-category), Integrations (MysticsHR connector with masked-key storage and last-sync status).
